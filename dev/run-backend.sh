#!/bin/bash
# Backend development server - handles database setup automatically
# Usage: ./dev/run-backend.sh
...
export METRICS_SIG_TEAMS_CONFIG="${METRICS_SIG_TEAMS_CONFIG:-${PROJECT_DIR}/dev/sig-teams.yaml}"
...
# Start server
echo "Starting FastAPI backend on http://localhost:${METRICS_SERVER_PORT}"
