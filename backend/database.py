...
from backend.config import MetricsConfig, get_config
...
class DatabaseManager:
    ...
    def __init__(self, config: MetricsConfig, logger: logging.Logger) -> None:
        """
    ...
    async def connect(self) -> None:
        ...
        self.logger.info(
            f"Connecting to PostgreSQL database: {db.user}@{db.host}:{db.port}/{db.name} (pool_size={db.pool_size})"
        )
    ...
    async def execute(self, query: str, *args: Any) -> str:
        ...
        try:
            async with self.pool.acquire() as connection:
                result: str = await connection.execute(query, *args)
    ...
    async def fetch(self, query: str, *args: Any) -> list[asyncpg.Record]:
        ...
        try:
            async with self.pool.acquire() as connection:
                results: list[asyncpg.Record] = await connection.fetch(query, *args)
...
def get_database_manager() -> "DatabaseManager":
    ...
    """
    config = get_config()
    logger = get_logger(name="backend.database")
